...
class NSGAII(EA):
    ...
    def initialise_x0(self) -> np.ndarray:
        ...
        # 2. Tight neighbors around each seed (exploitation)
        #    Split the tight budget evenly across seeds
        tight_per_seed = tenth // n_seeds
        idx = n_seeds
        for i, seed in enumerate(seeds):
            count = tight_per_seed if i < n_seeds - 1 else (n_seeds * tight_per_seed - (idx - n_seeds))
            end = idx + tight_per_seed
            population[idx:end] = seed + np.random.normal(0, 0.125, size=(tight_per_seed, self.n_params))
            idx = end
        ...
        for individual_a in range(len(fitness)):
            for individual_b in range(len(fitness)):
                ...
                if self.dominates(fitness[individual_a], fitness[individual_b]):
                    # Track that individual_a dominates individual_b
                ...
                # does candidate 2 dominate candidate 1?
                    ...
                    # Track that individual_a is dominated by individual_b
