...
class NSGAII(EA):
    ...
    def precharge(self) -> np.ndarray:
        ...
        n_pretrained = len(pretrained_weights)  # 560

        # Start with random population for full genotype (560 weights + 8 body params)
        population = np.random.uniform(self.min, self.max, (self.n_pop, self.n_params))

        # Elite individual: exact pretrained weights, random body params
        population[0, 8:n_pretrained+8] = pretrained_weights

        # 10% close to elite: small noise on weights, random body
        tenth = max(1, self.n_pop // 10)
        population[1:tenth, 8:n_pretrained+8] = pretrained_weights + np.random.normal(
            0, self.noise_std / 4, (tenth - 1, n_pretrained)
        )

        # Rest: larger noise on weights, random body
        population[tenth:, 8:n_pretrained+8] = pretrained_weights + np.random.normal(
            0, self.noise_std, (self.n_pop - tenth, n_pretrained)
        )
